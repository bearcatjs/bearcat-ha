    ...
    if (self.isMaster) {
        if (self.masterNode) {
            ...
            async.eachSeries(self.isolatedNodes, function(n, next) {
                ...
                if (!_node || _node.master == self.masterNode.name) {
                    return next();
                }
...
WatcherManager.prototype.onUnavailable = function(node) {
    ...
    this.addWatcherNode(node.options);
...
WatcherManager.prototype.addWatcherNode = function(opts) {
    ...
    this.nodes[watcherNode.name] = watcherNode;
...
WatcherManager.prototype.promoteHaMaster = function(nodes, index, callback) {
    ...
    node.makeMaster(function(err) {
        ...
        async.each(nodes, function(name, next) {
...
WatcherManager.prototype.updateData = function() {
    ...
    for (var name in this.nodes) {
        var watcherNode = this.nodes[name];
        if (watcherNode.available) {
            available.push(name);
        } else {
...
WatcherManager.prototype.updateNodesInfo = function(callback) {
    ...
    async.each(names, function(name, next) {
        var watcherNode = self.nodes[name];
        if (watcherNode.available) {
...
WatcherManager.prototype.updateHaState = function(data) {
    ...
    self.updateNodesInfo(function() {
        ...
        data.available.forEach(function(name) {
            ...
            var watcherNode = self.nodes[name];
            if (watcherNode.available && watcherNode.master === self.masterNode.name && watcherNode.linkedMaster) {
                slaves.push(name);
            } else {
                data.unavailable.push(name);

                if (watcherNode.master && !watcherNode.linkedMaster) {
                    logger.warn('%s linked to master: %s fail!', name, watcherNode.master);
                }
            }
        });
...
WatcherManager.prototype.getCloneOpts = function() {
    var opts = this.opts;
    var r = {};
